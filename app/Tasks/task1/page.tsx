...
// ── LOCAL TYPES ───────────────────────────────────────────────────────────────
// Define ChatMessage locally so we own the shape — avoids @/app/types mismatch.
...
// ── PIXEL GHOST SVG ───────────────────────────────────────────────────────────
function GhostSVG({ size = 44 }: { size?: number }): ReactElement {
  return (
...
// ── GENERIC LOG TABLE ─────────────────────────────────────────────────────────
// T extends Record<string, string> → every field is a string, no index-signature needed.
// cols: (keyof T & string)[] → compile-time enforcement that cols match T's keys.
interface LogTableProps<T extends Record<string, string>> {
  title: string;
  data: T[];
  cols: (keyof T & string)[];
  headers: string[];
...
function LogTable<T extends Record<string, string>>({
  title,
  data,
  cols,
  headers,
  gridCols,
}: LogTableProps<T>): ReactElement {
  const [search, setSearch] = useState('');

  const filtered = search
    ? data.filter(row => cols.some(col => (row[col] ?? '').toLowerCase().includes(search.toLowerCase())))
    : data;
  ...
  return (
    <div style={{ background: '#110707', border: '1px solid #2a0e0e', borderTop: '2px solid #6a0800', borderRadius: 2, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: gridCols, padding: '5px 11px', borderBottom: '1px solid #2a0e0e' }}>
        {headers.map(h => (
          <span key={h} style={{ fontSize: 11, color: '#cc6655', letterSpacing: 1, textTransform: 'uppercase' }}>{h}</span>
        ))}
      </div>
          ...
          filtered.map((row, i) => (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: gridCols, padding: '5px 11px', borderBottom: '1px solid rgba(42,14,14,0.4)' }}>
              {cols.map((col, ci) => (
                <span
                  key={col}
                  style={{ fontSize: 13, color: ci === 0 ? '#00e5b0' : '#00c490', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}
                >
                  {row[col]}
                </span>
              ))}
...
function TypingDots(): ReactElement {
  return (
    <div style={{ display: 'flex', gap: 5, alignItems: 'center', padding: '8px 4px' }}>
      {([0, 200, 400] as const).map(d => (
        <span
          key={d}
          style={{
            width: 6, height: 6, borderRadius: '50%', background: '#4a6a80',
            display: 'inline-block',
            animation: `tdot 1.2s ease-in-out ${d}ms infinite`,
          }}
        />
      ))}
    </div>
...
function IntroModal({ onDismiss }: IntroModalProps): ReactElement {
  ...
  const addTyping = useCallback((): number => {
    ...
    setMessages(m => [...m, { id, text: '', isTyping: true, show: true }]);
    return id;
  }, []);

  const removeTyping = useCallback((id: number): void => {
    setMessages(m => m.filter(x => x.id !== id));
  }, []);

  const addCard = useCallback((label: string | undefined, text: string): void => {
    const id = ++idRef.current;
    setMessages(m => [...m, { id, text, label, show: false }]);
    setTimeout(() => {
      setMessages(m => m.map(x => (x.id === id ? { ...x, show: true } : x)));
    }, 50);
  }, []);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      let tid = addTyping(); await wait(1300); if (cancelled) return; removeTyping(tid);
      ...
      tid = addTyping(); await wait(1100); if (cancelled) return; removeTyping(tid);
      ...
      tid = addTyping(); await wait(1200); if (cancelled) return; removeTyping(tid);
      addCard('ghost41_id › INSTRUCTIONS', 'Examine the Access Logs, Endpoint Logs, and VPN Network Logs on the main console.');
      await wait(800); if (cancelled) return;
      tid = addTyping(); await wait(1000); if (cancelled) return; removeTyping(tid);
      ...
      await wait(600); if (cancelled) return;
      tid = addTyping(); await wait(900); if (cancelled) return; removeTyping(tid);
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'rgba(4,1,1,0.94)', display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(6px)' }}>
      <div style={{ width: 430, maxWidth: '95vw', background: '#0b1825', border: '1px solid #1e4060', borderTop: '2px solid #00d4c8', borderRadius: 8, display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 0 60px rgba(0,212,200,0.12),0 30px 80px rgba(0,0,0,0.8)', animation: 'modalIn 0.5s cubic-bezier(.16,1,.3,1) forwards' }}>
        ...
        <div ref={logRef} style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 10, minHeight: 140, maxHeight: '52vh', overflowY: 'auto' }}>
          {messages.map(msg =>
            msg.isTyping ? (
              ...
              <div
                key={msg.id}
                style={{ background: '#0e2236', border: '1px solid #1a3550', borderRadius: 6, padding: '12px 14px', opacity: msg.show ? 1 : 0, transform: msg.show ? 'none' : 'translateY(8px)', transition: 'opacity 0.4s ease, transform 0.4s ease' }}
              >
                {msg.label != null && (
                  <div style={{ fontSize: 8, fontWeight: 700, letterSpacing: 2, color: '#00d4c8', textTransform: 'uppercase', marginBottom: 8, opacity: 0.8 }}>
                    {msg.label}
                  </div>
                )}
        ...
        <div style={{ padding: '12px 14px', borderTop: '1px solid #1a3550', background: '#0f2030', display: 'flex', justifyContent: 'flex-end', opacity: showBtn ? 1 : 0, transition: 'opacity 0.4s ease' }}>
          <button
            onClick={onDismiss}
            style={{ background: 'transparent', border: '1px solid #00d4c8', borderRadius: 4, color: '#00d4c8', fontSize: 10, fontWeight: 700, letterSpacing: 2, padding: '10px 24px', cursor: 'pointer', fontFamily: "'Courier New',monospace" }}
          >
            [ BEGIN INVESTIGATION ]
          </button>
        </div>
...
function GhostPanel({
  open, onClose, qStatuses, onQStatusChange, onAllDone,
}: GhostPanelProps): ReactElement {
  const [messages, setMessages] = useState<ChatMessage[]>([]);
  const [input, setInput] = useState('');
  const [qIdx, setQIdx] = useState(0);
  const [done, setDone] = useState(false);
  const [awaitingAnswer, setAwaiting] = useState(false);
  ...
  // Stable refs — async sequences always call the latest prop version
  ...
  const addMsg = useCallback((label: string | undefined, text: string, isUser = false): void => {
    const id = ++idRef.current;
    setMessages(m => [...m, { id, text, label, isUser }]);
  }, []);
  ...
  // Extracted to avoid inline ternary with precedence issues
  const isHintMsg = useCallback((msg: ChatMessage): boolean =>
    msg.label == null && (
      msg.text.startsWith('Cross') ||
      msg.text.startsWith('Check') ||
      msg.text.startsWith('Filter')
    ),
    []);

  const askQuestion = useCallback(async (idx: number): Promise<void> => {
    ...
    const q = QUESTIONS[idx]!;       // non-null: idx < QUESTIONS.length
    setAwaiting(true);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      let tid = addTyping(); await wait(1400); if (cancelled) return; removeTyping(tid);
      ...
      await wait(800); if (cancelled) return;
      tid = addTyping(); await wait(1100); if (cancelled) return; removeTyping(tid);
      if (!cancelled) void askQuestion(0);
    })();
  ...
  const send = useCallback(async (): Promise<void> => {
    ...
    const q = QUESTIONS[qIdx]!;     // non-null: qIdx is always within bounds here
    ...
    try {
      // Cast through unknown only at the network boundary — the only safe place
      result = (await verifyGhostAnswer('task1', q.qId, val)) as unknown as VerifyResult;
    } catch (err) {
  ...
  const dotColor = (cls: string): string => {
    if (cls === 'done' || cls === 'cur') return '#00d4c8';
    if (cls === 'fail') return '#ff2200';
    return '#1e4060';
  };

  const dots = QUESTIONS.map((_, i): string => {
  ...
  return (
    <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 360, zIndex: 960, background: '#0b1825', borderLeft: '2px solid #1e4060', display: 'flex', flexDirection: 'column', transform: open ? 'translateX(0)' : 'translateX(100%)', transition: 'transform 0.35s cubic-bezier(.16,1,.3,1)', boxShadow: '-8px 0 40px rgba(0,0,0,0.7)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: '1px solid #1e4060', background: '#0f2030', flexShrink: 0 }}>
        ...
        <button
          onClick={onClose}
          style={{ background: 'transparent', border: '1px solid #cc1100', borderRadius: 2, color: '#cc1100', fontSize: 9, fontWeight: 700, letterSpacing: '1.5px', padding: '5px 10px', cursor: 'pointer', fontFamily: "'Courier New',monospace", whiteSpace: 'nowrap' }}
        >
          ✕ CLOSE
        </button>
      </div>
      ...
      <div style={{ padding: '10px 12px', borderTop: '1px solid #1a3550', background: '#0f2030', flexShrink: 0 }}>
          ...
          {dots.map((cls, i) => (
            <span
              ...
              style={{
                ...
                background: dotColor(cls),
                boxShadow: cls === 'cur' ? '0 0 6px #00d4c8' : 'none',
                animation: cls === 'cur' ? 'blink 0.9s ease-in-out infinite' : 'none',
                display: 'inline-block',
              }}
            />
          ))}
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input
            ...
            onKeyDown={(e: React.KeyboardEvent<HTMLInputElement>) => { if (e.key === 'Enter') void send(); }}
...
export default function App(): ReactElement {
  ...
  const [csq, setCsq] = useState<boolean[]>([true, true, false, false, false]);
  ...
  const handleQStatus = useCallback((idx: number, s: QStatus): void => {
    setQStatuses(prev => {
      const next = [...prev] as QStatus[];
      next[idx] = s;
      return next;
    });
  ...
  const handleAllDone = useCallback((): void => {
  ...
  const openGhost = (): void => {
    setGhostOpen(true);
    if (!ghostStarted) setGhostStarted(true);
  };

  const csqColor = (i: number): React.CSSProperties => {
  ...
  return (
    <div style={{ height: '100vh', overflow: 'hidden', background: '#080404', fontFamily: "'Courier New',monospace", color: '#c8c0c0', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      {/* overlay effects */}
      <div style={{ position: 'fixed', inset: 0, background: 'repeating-linear-gradient(0deg,transparent,transparent 2px,rgba(0,0,0,0.07) 2px,rgba(0,0,0,0.07) 4px)', pointerEvents: 'none', zIndex: 900 }} />
      <div style={{ position: 'fixed', inset: 0, background: 'radial-gradient(ellipse at center,transparent 55%,rgba(0,0,0,0.75) 100%)', pointerEvents: 'none', zIndex: 899 }} />
      ...
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: 11 }}>
        ...
        {/* LOG TABLES — generic component, zero casting */}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 11 }}>
          ...
          <div style={{ background: '#110707', border: '1px solid #2a0e0e', borderTop: '2px solid #6a0800', borderRadius: 2, overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 11px', borderBottom: '1px solid #2a0e0e', background: '#160909' }}>
              <div style={{ display: 'flex' }}>
                {(['Overview', 'Timeline', 'Anomalies'] as const).map(tab => (
                  <button
                    key={tab}
                    onClick={() => setActiveTab(tab)}
                    style={{ background: 'none', border: 'none', borderRight: '1px solid #2a0e0e', color: activeTab === tab ? '#00e5b0' : '#3a2222', fontSize: 9, fontWeight: 700, letterSpacing: 2, cursor: 'pointer', fontFamily: "'Courier New',monospace", padding: '3px 9px', textTransform: 'uppercase' }}
                  >
                    {tab}
                  </button>
                ))}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 0.85fr 1.4fr', padding: '5px 11px', borderBottom: '1px solid #2a0e0e' }}>
              {(['User', 'Login Time', 'File Accessed'] as const).map(h => (
                <span key={h} style={{ fontSize: 11, color: '#cc6655', letterSpacing: 1, textTransform: 'uppercase' }}>{h}</span>
              ))}
            </div>
            <div style={{ maxHeight: 210, overflowY: 'auto' }}>
          ...
          <div style={{ background: '#110707', border: '1px solid #2a0e0e', borderTop: '2px solid #6a0800', borderRadius: 2, overflow: 'hidden' }}>
            <div style={{ padding: '7px 12px', borderBottom: '1px solid #2a0e0e', background: '#160909', fontSize: 9, fontWeight: 700, letterSpacing: '2.5px', color: '#ff2200', textTransform: 'uppercase' }}>
              Investigation Progress
            </div>
            <div style={{ padding: 12 }}>
              {(
                [
                  { num: '01', text: 'Who is responsible for the breach?' },
                  { num: '02', text: 'How much data was exfiltrated?' },
                  { num: '03', text: 'What external IP was tunneled to at 21:03?' },
                ] as const
              ).map((item, i) => (
      ...
      <div style={{ background: '#0d0606', borderTop: '1px solid #6a0800', padding: '6px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        ...
        <div style={{ fontSize: 10, color: '#3a2222', letterSpacing: 1 }}>
          NEXT UNLOCK : <span style={{ color: '#00e5b0' }}>Incident Records Module</span>
        </div>
      </div>
      ...
      <div
        onClick={openGhost}
        style={{ position: 'fixed', bottom: 70, right: 24, zIndex: 950, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, cursor: 'pointer', userSelect: 'none', animation: 'ghost-float 3s ease-in-out infinite' }}
      >
        <GhostSVG size={60} />
        <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: 2, color: '#00d4c8', background: 'rgba(11,24,37,0.9)', border: '1px solid #1e4060', borderRadius: 3, padding: '3px 8px', whiteSpace: 'nowrap', textTransform: 'uppercase', animation: 'label-pulse 2s ease-in-out infinite' }}>
          ASK ME!
        </span>
      </div>
      ...
      {showSuccessModal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'modalIn 0.5s ease' }}>
          <div style={{ background: '#0a1520', border: '1px solid #00d4c8', padding: '40px', borderRadius: '4px', textAlign: 'center', maxWidth: '400px', boxShadow: '0 0 40px rgba(0,212,200,0.2)' }}>
            <h2 style={{ color: '#00d4c8', fontSize: '24px', letterSpacing: '3px', textTransform: 'uppercase', marginBottom: '16px', textShadow: '0 0 20px rgba(0,212,200,0.5)' }}>
              Task Complete
            </h2>
            <p style={{ color: '#c8dce8', fontSize: '14px', marginBottom: '32px', lineHeight: '1.6' }}>
              Log analysis is complete. The evidence has been extracted. Return to the dashboard to proceed.
            </p>
            <a
              href="/dashboard"
              style={{ display: 'inline-block', background: 'transparent', border: '1px solid #00d4c8', color: '#00d4c8', textDecoration: 'none', padding: '12px 24px', fontSize: '12px', fontWeight: 'bold', letterSpacing: '2px', cursor: 'pointer', transition: 'all 0.3s' }}
            >
              RETURN TO DASHBOARD
            </a>
          </div>
