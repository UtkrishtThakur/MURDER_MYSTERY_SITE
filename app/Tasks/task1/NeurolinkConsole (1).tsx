...
interface Question   { q: string; hint: string; ans: string[]; display: string; stId: string; }
...
const QUESTIONS: Question[] = [
  {q:"Who is responsible for the data breach?",             hint:"Cross-reference access logs and endpoint transfer anomalies.", ans:["rishab sen","rishab.sen","rishab"], display:"rishab sen", stId:"st-q1"},
  {q:"How much data was exfiltrated in total?",             hint:"Check the endpoint Transfer column for large anomalies.",      ans:["18.6gb","18.6 gb","18.6 gigabytes","18.6"], display:"18.6 GB", stId:"st-q2"},
  {q:"What external IP did the suspect tunnel to at 21:03?",hint:"Filter VPN logs for external connections.",                   ans:["203.0.113.45"], display:"203.0.113.45", stId:"st-q3"},
];
  ...
  return (
    <div style={{ background:"#110707", border:"1px solid #2a0e0e", borderTop:"2px solid #6a0800", borderRadius:2, overflow:"hidden" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:gridCols, padding:"5px 11px", borderBottom:"1px solid #2a0e0e" }}>
        {headers.map(h => <span key={h} style={{ fontSize:10, color:"#554444", letterSpacing:1, textTransform:"uppercase" }}>{h}</span>)}
      </div>
      <div style={{ maxHeight:210, overflowY:"auto" }}>
        {filtered.length === 0
          ? <div style={{ color:"#3a2222", fontSize:11, padding:"8px 11px" }}>— no results —</div>
          : filtered.map((row, i) => (
            <div key={i} style={{ display:"grid", gridTemplateColumns:gridCols, padding:"7px 11px", borderBottom:"1px solid rgba(42,14,14,0.4)" }}>
              {cols.map((c, ci) => (
                <span key={c} style={{ fontSize:12, color: ci === 0 ? "#00ffcc" : "#00c49a", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", fontWeight: ci === 0 ? 600 : 400 }}>
                  {row[c]}
                </span>
              ))}
            </div>
...
const GhostPanel = ({ open, onClose, qStatuses, onQStatusChange, onAllDone }: GhostPanelProps) => {
  ...
  const send = useCallback(async () => {
    ...
    const ok = q.ans.some(a => val.toLowerCase() === a.toLowerCase());
    await delay(200);
    const tid = addTyping(); await delay(600); removeTyping(tid);
    if (ok) {
      addMsg(undefined, `✓ Confirmed — ${q.display}`);
      ...
      addMsg(undefined, "✗ Incorrect. Look more carefully at the logs and try again.");
      await delay(300);
      setAwaitingAnswer(true);
    }
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:11 }}>
          ...
          <div style={{ background:"#110707", border:"1px solid #2a0e0e", borderTop:"2px solid #6a0800", borderRadius:2, overflow:"hidden" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 0.85fr 1.4fr", padding:"5px 11px", borderBottom:"1px solid #2a0e0e" }}>
              {["User","Login Time","File Accessed"].map(h => <span key={h} style={{ fontSize:10, color:"#554444", letterSpacing:1, textTransform:"uppercase" }}>{h}</span>)}
            </div>
            <div style={{ maxHeight:210, overflowY:"auto" }}>
              {COMBINED.map((row, i) => (
                <div key={i} style={{ display:"grid", gridTemplateColumns:"1fr 0.85fr 1.4fr", padding:"7px 11px", borderBottom:"1px solid rgba(42,14,14,0.4)" }}>
                  <span style={{ fontSize:12, color:"#00ffcc", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", fontWeight:600 }}>{row.user}</span>
                  <span style={{ fontSize:12, color:"#00c49a", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{row.loginTime}</span>
                  <span style={{ fontSize:12, color:"#00c49a", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{row.fileAccessed}</span>
                </div>
              ))}
            </div>
          </div>
